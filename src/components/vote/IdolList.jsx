...
/** 아이돌 리스트 컴포넌트 props (기본형과 투표형을 함께 사용하므로 자세한 사용법은 파일 마지막을 참고해주세요.)
 * @param {number} id - [기본형 | 투표형] 아이돌 id
 * @param {string} imgUrl - [기본형 | 투표형] 이미지
 * @param {string} rank - [기본형 | 투표형] 순위
 * @param {string} group - [기본형 | 투표형] 아이돌 그룹
 * @param {string} name - [기본형 | 투표형] 아이돌 이름
 * @param {number} votes - [기본형 | 투표형] 투표 수 (string으로 사용 시, 포맷 설정 함수에서 오류가 발생합니다.)
 * @param {number} selectedId - [투표형] 현재 선택된 아이돌 id
 * @param {function} onChange - [투표형] selectedId 변경시 값 저장 이벤트 핸들러
 */
...
const IdolImg = ({ selected }) => {
  const { imgUrl } = useContext(IdolContext);

  return (
    <div className='border-brand-1 aspect-square w-20 rounded-full border-1 object-cover p-1'>
      <div className='relative h-full w-full overflow-hidden rounded-full'>
        <img className='block h-full w-full object-cover' src={imgUrl} />
        {selected && <Selected />}
      </div>
    </div>
  );
